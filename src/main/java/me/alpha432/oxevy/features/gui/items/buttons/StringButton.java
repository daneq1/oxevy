...
import me.alpha432.oxevy.util.models.Timer;
...
public class StringButton
    ...
    public void drawScreen(GuiGraphics context, int mouseX, int mouseY, float partialTicks) {
        RenderUtil.rect(context, this.x, this.y, this.x + (float) this.width + 7.4f, this.y + (float) this.height - 0.5f, this.getState() ? (!this.isHovering(mouseX, mouseY) ? Oxevy.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().color.getValue().getAlpha()) : Oxevy.colorManager.getColorWithAlpha(y, ClickGuiModule.getInstance().topColor.getValue().getAlpha())) : (!this.isHovering(mouseX, mouseY) ? 0x11555555 : -2007673515));
        if (this.isListening) {
            int textColor = ColorUtil.toRGBA(ClickGuiModule.getInstance().color.getValue());
            drawString(this.currentString.string() + getIdleSign(), this.x + 2.3f, this.y - 1.7f - (float) OxevyGui.getClickGui().getTextOffset(), textColor);
        } else {
            int textColor = ColorUtil.toRGBA(ClickGuiModule.getInstance().color.getValue());
            drawString((this.setting.getName().equals("Buttons") ? "Buttons " : (this.setting.getName().equals("Prefix") ? "Prefix  " + ChatFormatting.GRAY : "")) + this.setting.getValue(), this.x + 2.3f, this.y - 1.7f - (float) OxevyGui.getClickGui().getTextOffset(), textColor);
        }
    }
